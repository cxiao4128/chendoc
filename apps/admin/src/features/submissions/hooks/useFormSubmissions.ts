/**
 * features/submissions/hooks/useFormSubmissions.ts
 * 提交记录 hooks（从 forms 迁移）
 */
import { computed, onMounted, ref } from "vue";
import { useRoute, useRouter } from "vue-router";
import type { FormField, FormItem, IpStats, SubmissionItem } from "@/services/api/forms.api";
...
export function useFormSubmissions() {
  ...
  const inputFields = computed(() => form.value?.fields.filter((field: FormField) => field.type !== "section") || []);
  ...
  async function exportData(format: FormExportFormat) {
    ...
    try {
      ...
      const fields = response.form.fields.filter((field: FormField) => field.type !== "section");
      ...
      if (format === "csv") {
        const headers = ["提交时间", "来源摘要", "浏览器信息", ...fields.map((field: FormField) => field.label)];
        const rows = response.submissions.map((submission: SubmissionItem) => {
          const row: string[] = [formatFormDate(submission.submittedAt), submission.ip, submission.userAgent || ""];
      ...
      fields.forEach((field: FormField) => { html += `<th>${escapeHtml(field.label)}</th>`; });
      html += "</tr></thead><tbody>";
      response.submissions.forEach((submission: SubmissionItem) => {
  ...
  async function doDeleteSubmission() {
    ...
    try {
      ...
      submissions.value = submissions.value.filter((s: SubmissionItem) => s.id !== deletingSubmissionId.value);
