...
import {
  ...
  getManagedUserPasswordView, listManagedUsers, promoteManagedUser, resetManagedUserPassword
} from "./users.service.js";
import { listOperationLogs } from "./logs.service.js";
import { exportSystemConfig, getSystemOverview, runSystemMaintenanceAction } from "./maintenance.service.js";

export { listSettings, setSetting } from "./core.service.js";
export { getSiteConfig, saveSiteConfig } from "./site.service.js";
export { getR2Config, saveR2Config, testR2Connection } from "./storage.service.js";
export { listOperationLogs } from "./logs.service.js";
export {
  deleteManagedUser, disableManagedUser, enableManagedUser, getManagedUser,
  getManagedUserPasswordView, listManagedUsers, promoteManagedUser, resetManagedUserPassword
} from "./users.service.js";
export { exportSystemConfig, getSystemOverview, runSystemMaintenanceAction } from "./maintenance.service.js";
...
export async function settingsRoutes(app: FastifyInstance) {
  ...
  app.get("/api/settings/system/export", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "system.export_config", targetType: "system", targetId: "config", ...auditMetaFromRequest(request) });
  ...
  app.post("/api/settings/system/actions/:action", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: `system.${params.action}`, targetType: "system", targetId: params.action, ...auditMetaFromRequest(request) });
    return { result };
  });
  ...
  app.post("/api/admin/users/:id/promote", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "user.promote_admin", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
    return { user };
  });
  ...
  app.post("/api/admin/users/:id/disable", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "user.disable_login", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
    return { user };
  });
  ...
  app.post("/api/admin/users/:id/enable", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "user.enable_login", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
    return { user };
  });
  ...
  app.delete("/api/admin/users/:id", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "user.delete", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
    return { ok: true };
  });
  ...
  app.get("/api/admin/users/:id/password", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "user.password.view", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
    return { password };
  });
  ...
  app.post("/api/admin/users/:id/password", { preHandler: dangerousAdmin }, async (request, reply) => {
    ...
    try {
      ...
      await writeAuditLog({ userId: request.user!.id, action: "user.password.reset", targetType: "user", targetId: String(params.id), ...auditMetaFromRequest(request) });
      return { user };
    } catch (error) {
  ...
  app.post("/api/settings/site", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "settings.site.update", targetType: "settings", targetId: "site", ...auditMetaFromRequest(request) });
    return { config };
  });
  ...
  app.post("/api/settings/storage/r2", { preHandler: dangerousAdmin }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: "settings.r2.update", targetType: "settings", targetId: "r2", ...auditMetaFromRequest(request) });
    return { config };
  });
  ...
  app.post("/api/settings/storage/r2/test", { preHandler: adminOnly }, async (request) => {
    ...
    await writeAuditLog({ userId: request.user!.id, action: body.upload ? "settings.r2.test_upload" : "settings.r2.test", targetType: "settings", targetId: "r2", ...auditMetaFromRequest(request) });
    return result;
  });
}
