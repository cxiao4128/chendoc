...
| ID | 严重程度 | 问题 | 状态 | 修复日期 | 备注 |
|----|----------|------|------|----------|------|
| H-1 | 🟠 高危 | HMAC 密钥派生未使用 HKDF | ✅ 已修复 | 2026-06-30 | `packet.ts:298` 已使用 `hkdfSync` |
| M-1 | 🟡 中危 | 文件上传双扩展名绕过风险 | ✅ 已修复 | 2026-06-30 | `uploads.service.ts` 增加最终扩展名检测 |
| M-2 | 🟡 中危 | 会话刷新并发容错 | ✅ 已修复 | 2026-06-30 | `session.service.ts` 已实现乐观锁 |
| L-1 | 🟢 低危 | isSuperAdminDoc 类型宽松 | ✅ 已修复 | 2026-06-30 | schema.mysql/ts 均使用 boolean 类型 |
| L-2 | 🟢 低危 | 前端路由隔离依赖前端判断 | ✅ 已确认 | 2026-06-30 | 后端 API 有 `documentAccess.ts` 兜底 |
